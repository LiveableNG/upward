...
export function filterLocationOptions(query: string, limit = 40): RequestHomeLocationOption[] {
  const needle = query.trim().toLowerCase()

  if (!needle) {
    return REQUEST_HOME_LOCATION_OPTIONS.slice(0, limit)
  }

  return REQUEST_HOME_LOCATION_OPTIONS.filter((option) => option.searchText.includes(needle)).slice(
    0,
    limit,
  )
}

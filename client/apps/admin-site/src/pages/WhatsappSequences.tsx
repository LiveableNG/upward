...
export default function WhatsappSequences({ token }: WhatsappSequencesProps) {
  ...
  const fetchLogs = async () => {
    ...
    try {
      const query = new URLSearchParams({
        limit: '200', // keeping limit at 200 per page for sequence
        page: page.toString(),
        stage: activeStage,
  ...
  const fetchPreview = async () => {
    ...
    try {
      const res = await fetch(`${import.meta.env.VITE_API_URL}/admin/whatsapp-sequences/preview?stage=${activeStage}&name=${previewName}`, {
        headers: { Authorization: `Bearer ${token}` },
      })
  ...
  const handleBatchRetry = async () => {
    ...
    if (!confirm(`Are you sure you want to retry ${failedLogs.length} failed messages?`)) return
  ...
  return (
    <div className="page-container">
      <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '32px' }}>
        <div>
          <h1 className="section-title" style={{ margin: '0 0 8px 0' }}>WhatsApp Sequences</h1>
          <p style={{ color: 'var(--text-muted)', margin: 0 }}>Manage and preview automated WhatsApp onboarding messages.</p>
        </div>
        ...
        {showPreview && (
          <div className="card" style={{ display: 'flex', flexDirection: 'column', padding: 0, overflow: 'hidden' }}>
            ...
            <div style={{ padding: '20px', flex: 1 }}>
              ...
              <div style={{ background: '#f0fdf4', border: '1px solid #bbf7d0', borderRadius: 'var(--radius-md)', padding: '16px', minHeight: '200px', position: 'relative' }}>
                {previewLoading ? (
                  <div style={{ color: '#166534', textAlign: 'center', marginTop: '40px' }}><RefreshCcw size={20} className="loader" style={{margin: '0 auto', borderColor: '#bbf7d0', borderTopColor: '#166534'}} /></div>
                ) : (
                  <div style={{ whiteSpace: 'pre-wrap', color: '#14532d', fontSize: '14px', lineHeight: '1.5' }}>
                    {previewText}
                  </div>
                )}
              </div>
                ...
                <tbody>
                  {loading ? (
                    ...
                    logs.map(log => {
                      ...
                      const contact = log.user?.phone || log.user?.email || '-'

  ...
  async generatePdf(data: RentDepositReceiptData): Promise<Buffer> {
    return new Promise((resolve, reject) => {
      ...
      doc
        ...
        .fontSize(16)
        .fillColor('#FBF3EE')
        .text('UPWARD', BRAND_X + ICON_SIZE + 8, BRAND_Y - 1)

      const amountPrefix = isCredit ? '+' : '-'
      const displayAmount = `${amountPrefix}${currency} ${Number(data.amount || 0).toLocaleString('en-US', {
      ...
      doc
        ...
        .fontSize(28)
        .fillColor('#FBF3EE')
        .text(displayAmount, BRAND_X, BRAND_Y + 36, { width: W - 64, align: 'left' })

      const PILL_Y = BRAND_Y + 84
      const PILL_W = 140
      const PILL_H = 22

      const statusText = isCredit ? 'DEPOSIT RECEIVED' : 'DEPOSIT APPLIED'
      doc.save()
      doc.roundedRect(BRAND_X, PILL_Y, PILL_W, PILL_H, 11).clip()
      doc.rect(BRAND_X, PILL_Y, PILL_W, PILL_H).fillOpacity(0.2).fill('#FBF3EE')

...
export class UnstructuredPipelineService {
  ...
  async processDocument(
    ...
    const rawApiUrl = this.configService.get<string>(
    ...
    try {
      if (rawApiUrl.includes('platform-api.transform.unstructured.io')) {
        return await this.processViaTransformJobsApi(
          rawApiUrl,
          apiKey,
          fileBuffer,
          mimeType,
  ...
  private async processViaTransformJobsApi(
    ...
    if (!response.ok) {
      throw new Error(
        `Transform API job creation failed (${response.status}): ${await response.text()}`,
      )
    }
  ...
  private async processViaDirectApi(
    ...
    if (!response.ok) {
      throw new Error(
        `Unstructured API returned status ${response.status}: ${await response.text()}`,
      )
    }

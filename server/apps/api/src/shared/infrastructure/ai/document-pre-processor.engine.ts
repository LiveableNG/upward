...
export class DocumentPreProcessorEngine {
  ...
  async preProcess(fileBuffer: Buffer, mimeType: string, fileName: string): Promise<{
            ...
            const values = Array.isArray(row.values)
              ? row.values.slice(1).map(val => {
                  ...
                  if (typeof val === 'object') {
                    return (val as any).result !== undefined ? String((val as any).result) : JSON.stringify(val);
                  }
    ...
    // Scanned Images
    if (lowerMime.includes('image/') || fileName.endsWith('.png') || fileName.endsWith('.jpg') || fileName.endsWith('.jpeg')) {
      return {
        hasVisuals: true,

...
export class AiParseDocumentUseCase {
  ...
  async execute(input: AiParseDocumentInput): Promise<AiParsedRow[]> {
    ...
    if (pathway === 'hybrid') {
      ...
      if (isUnstructuredEnabled) {
        ...
        if (extractedText && extractedText.trim().length > 0 && !extractedText.startsWith('[')) {
          ...
          this.logger.warn(
            'Unstructured processing returned no direct text elements. Self-healing: Falling back to direct layout/vision parsing.',
          )
        }
      } else {
    ...
    return rawRows.map((row, idx) => {
      ...
      const hasTenantName = !!(
        sanitized.tenantFirstName?.trim() || sanitized.tenantLastName?.trim()
      )
      const hasCommercialName = !!sanitized.tenantCommercialName?.trim()
      if (
        (hasTenantName || hasCommercialName) &&
        (!sanitized.tenantEmail || sanitized.tenantEmail.trim() === '')
      ) {
      ...
      sanitized.unitRentAmountPaid = sanitized.unitRentAmountPaid
        ? Number(sanitized.unitRentAmountPaid)
        : 0

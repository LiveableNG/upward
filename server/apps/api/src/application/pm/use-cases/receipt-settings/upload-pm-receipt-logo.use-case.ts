...
@Injectable()
export class UploadPmReceiptLogoUseCase {
  constructor(
  ...
  async execute(pmUuid: string, base64Data: string, contentType: string) {
    ...
    const key = `pm/${pm.uuid}/receipt-settings/${filename}`
    ...
    const publicUrl = `${baseUrl}/api/v1/public/documents/pm/receipt-settings/logo/${pm.uuid}/${filename}`

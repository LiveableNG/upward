...
function intervalToDuration(start: Date, end: Date) {
...
function formatDurationParts(duration: { years: number; months: number; days: number }, inWords = false): string {
  const parts: string[] = [];
...
export function formatTimeframeUntilDate(dateValue: any, referenceDate: Date = new Date()): string {
  ...
  const duration = intervalToDuration(referenceDate, targetDate);
